...
from scripts.utils import default, cast_tuple
from scripts.unet_modules import SinusoidalPositionEmbeddings, Upsample, Downsample, ResnetBlock, ResnetBlock_BigGAN
...
class Unet(pl.LightningModule):
    ...
    def __init__(
        ...
        block_klass = partial(ResnetBlock_BigGAN, groups = resnet_block_groups)
        ...
        for ind, ((dim_in, dim_out), layer_full_attn) in enumerate(zip(in_out, full_attn)):
            ...
            self.downs.append(nn.ModuleList([
                block_klass(dim_in, dim_in, time_emb_dim = time_dim, updown='identity'),
                block_klass(dim_in, dim_in, time_emb_dim = time_dim, updown='Downsample' if not is_last else 'identity'),
                attn_klass(dim_in)
            ]))

        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim = time_dim, updown='identity')
        self.mid_attn = FullAttention(mid_dim)
        self.mid_block2 = block_klass(mid_dim, mid_dim, time_emb_dim = time_dim, updown='identity')
        ...
        for ind, ((dim_in, dim_out), layer_full_attn) in enumerate(zip(reversed(in_out), reversed(full_attn))):
            ...
            self.ups.append(nn.ModuleList([
                block_klass(dim_out + dim_in, dim_out, time_emb_dim = time_dim, updown='identity'),
                block_klass(dim_out, dim_out, time_emb_dim = time_dim, updown='Upsample' if not is_last else 'identity'),
                attn_klass(dim_out)
            ]))
        ...
        self.final_res_block = block_klass(dim * 2, dim, time_emb_dim = time_dim, updown='identity')
